...
def srcs():
    return stgfunc.src("raws/[SubsPlease] Bocchi the Rock! - 03 (1080p) [1F17BE65].mkv", depth=16)


def denoise(src_in):
    den = EoEfunc.denoise.BM3D(src_in, [1, 0])
    den = ccd.ccd(den, 1)
    out = den
    return out
...
def dehalo(src_clip):
    shalod = vsdehalo.fine_dehalo(src_clip, rx=2, darkstr=0, show_mask=False)

    aa = vapoursynth.core.bilateral.Gaussian(src_clip, 0.5)
    i_aa = vsaa.upscaled_sraa(aa, 2)
    halo_mask = jvsfunc.dehalo_mask(i_aa, expand=1, iterations=0, brz=200)
